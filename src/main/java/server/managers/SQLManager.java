...
public class SQLManager {
    ...
    //createSpaceMarine
    public void createSpaceMarine(PriorityQueue<SpaceMarine> collection) {

        try (Connection connection = sqlconnector.getConnectToSQL()) {
            PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO spacemarine (NAME, COORDINATE_X, COORDINATE_Y, CREATION_DATE, HEALTH, HEART_COUNT, CATEGORY, MELEE_WEAPON, CHAPTER_NAME, CHAPTER_WORLD) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
              for (SpaceMarine spaceMarine : collection) {
                  ...
                  preparedStatement.setString(7, spaceMarine.getCategory().getCategory());
                  preparedStatement.setString(8, spaceMarine.getMeleeWeapon().getWeapon());
                  preparedStatement.setString(9, spaceMarine.getChapter().getName());
                  preparedStatement.setString(10, spaceMarine.getChapter().getWorld());
    ...
    public void updateSpaceMarine(SpaceMarine spaceMarine) {
        String SQL = "UPDATE Space_Marine SET NAME=?, COORDINATE_X=?, COORDINATE_Y=?, HEALTH=?, HEART_COUNT=?, ASTARTES_CATEGORY=?, MELEE_WEAPON=?, CHAPTER_NAME=?, CHAPTER_WORLD=? WHERE WHERE ID = ?";
        try (Connection conn = sqlconnector.getConnectToSQL()) {
...
//                    spaceMarines.add(new SpaceMarine(rs.getLong("ID"), rs.getString("NAME"), coordinates,  rs.getDate("CREATION_DATE").toLocalDateTime(), rs.getLong("HEALTH"), rs.getInt("HEART_COUNT"), rs.getString("ASTARTES_CATEGORY"),MeleeWeapon.getValue(rs.getString("MELEE_WEAPON")), new Chapter(rs.getString("CHAPTER_WORLD"), rs.getString("CHAPTER_NAME"))));
                }
                rs.close();
